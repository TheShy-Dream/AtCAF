...
def eval_ood_2(results, truths, exclude_zero=False):
    results = results.cpu().detach()
    truths = truths.cpu().detach()
    non_zeros = np.array([i for i, e in enumerate(truths) if e != 0])

    binary_truth_non0 = truths[non_zeros]
    binary_preds_non0 = np.argmax(results[non_zeros],1)

    binary_truth_has0 = truths
    binary_preds_has0 = np.argmax(results,1)

    binary_truth_non0=change_to_classify(binary_truth_non0,2)
    binary_truth_has0=change_to_classify(binary_truth_has0,2)
    ...
    print("F1 score all/non0: {}/{} over {}/{}".format(f1_1, f1_2,
                                                       binary_truth_has0.shape[0], binary_truth_non0.shape[0]))
    return {"default": np.round(acc_2, 4) * 100,"acc_2_none0": np.round(acc_2_non0, 4) * 100,
         "f1_1": np.round(f_score, 4) * 100, "f1_2": np.round(f_score_non0, 4) * 100}

def eval_ood_7(results, truths, exclude_zero=False):
    results = results.cpu().detach()
    truths = truths.cpu().detach()
    test_pred_a7 = np.argmax(results,1)
    truths=change_to_classify(truths,7)
    print(test_pred_a7,truths)
    acc7 = accuracy_score(test_pred_a7,truths)
    return {'acc7': acc7}
